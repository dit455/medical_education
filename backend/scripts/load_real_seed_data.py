"""One-off: replaces the placeholder dev data (test institutions/courses/
subjects/categories/exam-sems) with the real Puducherry medical education
board dataset. Run once: backend/venv/Scripts/python.exe backend/scripts/load_real_seed_data.py
"""
...
WIPE_STATEMENTS = [
    ...
    "DELETE FROM tbl_student_marks",
    "DELETE FROM tbl_student_master",
    "DELETE FROM tbl_pending_changes",
    "DELETE FROM tbl_course_subject_map",
    "DELETE FROM tbl_inst_course_map",
    "DELETE FROM tbl_subject_master",
    "DELETE FROM tbl_course_master",
    "DELETE FROM tbl_inst_master",
    "DELETE FROM tbl_category_master",
    "DELETE FROM tbl_exam_sem_master",
]
...
def main():
    ...
    try:
        ...
        for sql in [EXAM_SEM_SQL, CATEGORY_SQL, INST_SQL, COURSE_SQL, SUBJECT_SQL,
                    INST_COURSE_MAP_SQL, COURSE_SUBJECT_MAP_SQL]:
            cursor.execute(sql)
